...
def get_label_config(project):
    filepath = os.path.join(_manager.model_param_dir, f'model_param/{project}.json')
    label_config = None
    
    try:
        with open(filepath, mode='r', encoding='utf-8') as file_obj:
            label_config = json.load(file_obj)
    except:
        label_config = None

    return label_config

@_server.route('/eval', methods=['POST'])
@exception_handler
def _eval():
    data = request.json
    ...
    annotation_result = _manager.eval(project_id, image, result_prefix)
    return jsonify(annotation_result)

@_server.route('/init', methods=['POST'])
@exception_handler
def _init():
    data = request.json
    project_id = data.get('project_id')
    label_config = get_label_config(project_id)
    if label_config is None:
        return jsonify({'rc': 1})
    else:
        logger.debug(label_config)
        project = label_config.get('project')
        schema = label_config.get('label_config', '')
        force_reload = False
        hostname = label_config.get('hostname', '')  # host name for uploaded files and building urls
        access_token = label_config.get('access_token', '')  # user access token to retrieve data
