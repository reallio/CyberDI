...
class LabelStudioMLManager(object):
    ...
    @classmethod
    def initialize(
        cls, model_class, model_dir=None, model_param_dir=None, redis_host='localhost', redis_port=6379, redis_queue='default',
        **init_kwargs
    ):
        ...
        cls.model_param_dir = model_param_dir
    ...
    def _get_latest_job_result_from_workdir(cls, project):
        if cls.model_dir is None: return
    ...
    @classmethod
    def eval(
        cls, project_id, image, result_prefix
    ):
        m = None
        for (k1, k2), v in cls._current_model.items():
            if project_id == k1.split('.', 1)[0]:
                m = v
        if m is not None:
            result = m.model.eval(image, result_prefix)
        else:
            result = {"rc": 1}
        return result
